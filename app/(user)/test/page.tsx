...
const TestJwtPage = () => {
    ...
    const handleLogin = async () => {
        const email = "artvandeth@gmail.com";
        const password = "Panda101001$8$8";
    ...
    const handleUpdate = async () => {
        const body = {
            name: "Product Update By Vanndeth"
        }
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}products/${88}/`,{
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
   ...
   const handleRefreshToken = async () => {
    ...
    return (
        <main className="h-screen grid place-content-center">
            <h1 className="text-4xl font-bold text-center">Test Handle Login</h1>
            <button className="my-4 px-10 py-3 bg-blue-600 rounded-xl text-gray-100 text-3xl"
                    onClick={handleLogin}>Login
            </button>
            {/*refresh token automatically*/}
            <button className="my-4 px-10 py-3 bg-blue-600 rounded-xl text-gray-100 text-3xl"
                    onClick={handleUpdate}>Partial Update
            </button>
